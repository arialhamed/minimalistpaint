...
class MinimalisticPaint(tk.Canvas):
    def __init__(self, parent, *args, **kwargs):
        ...
        self.extension = ".png"
        self.in_shape = "line"
        self.in_color = "#ff0000" # clear is None
...
##        self.toolbar.geometry('+{}+0'.format(int(self.toolbar.winfo_rootx()/2-self.toolbar.winfo_x()/2)))
        self.toolbar.geometry('+{}+0'.format(100))
...
##        self.parent.geometry('+0+0')
    ...
    def B1_Motion(self, pos):
        self.main_input = [(pos.x, pos.y), self.in_shape]
        self.pos_line += [tuple([pos.x,pos.y])]
    def B3_Motion(self, pos):
        return
    
    def ButtonRelease_1(self, pos):
        self.main_input = [(pos.x, pos.y), '']
        self.draw_image.line(self.pos_line,
                             fill=("#ffffff" if self.in_mode == "erase" else self.in_color),
                             width=self.in_size,
                             joint="curve")#, outline="white" if self.in_mode == "erase" else self.in_color)
        self.pos_line = []
    def ButtonRelease_3(self, pos):
        return
    
    def draw(self, pos, shape, size=0):
        size = int(self.in_size)
        self.pos2 = tuple(self.pos2)
        if shape == "line":
            #size = int(self.in_size/2)
            self.canvas.create_line([pos[0], pos[1], self.pos2[0], self.pos2[1]],
                                    fill=("#ffffff" if self.in_mode == "erase" else self.in_color),
                                    width=size,
                                    capstyle="round")#, outline="white" if self.in_mode == "erase" else self.in_color)
##            self.pos_line += [pos]
        if shape == "rectangle":
    ...
    def second_update(self):
        self.pos2 = self.main_input[0]
        #self.parent.after(self.framerate, self.third_update)
